...
object HelloWorldSpec extends DefaultRunnableSpec {
  def spec = suite("Number-List Spec")(
    testM("::") {
      check(
        Gen.listOf(Gen.int(Int.MinValue, Int.MaxValue)),
        Gen.int(Int.MinValue, Int.MaxValue),
        Gen.string(Gen.char(Char.MinValue, Char.MaxValue))
      ) { (list, item1, item2) =>
    ...
    testM(":::") {
      check(
        Gen.listOf(Gen.int(Int.MinValue, Int.MaxValue)),
        Gen.listOf(Gen.int(Int.MinValue, Int.MaxValue)),
        Gen.listOf(Gen.string(Gen.char(Char.MinValue, Char.MaxValue)))
      ) { (list1, list2, list3) =>
    ...
    testM("collect") {
      check(Gen.listOf(Gen.option(Gen.int(Int.MinValue, Int.MaxValue))), Gen.listOf(Gen.string(Gen.char(Char.MinValue, Char.MaxValue)))) {
        (list1, list2) =>
          val number1 = OpsConvertList.fromList(list1)
          val number2 = OpsConvertList.fromList(list2)
    ...
    testM("headOption") {
      check(Gen.listOf(Gen.int(Int.MinValue, Int.MaxValue)), Gen.listOf(Gen.string(Gen.char(Char.MinValue, Char.MaxValue)))) {
        (list1, list2) =>
          val number1 = OpsConvertList.fromList(list1)
          val number2 = OpsConvertList.fromList(list2)
    ...
    testM("exists") {
      check(
        ...
        val number1 = OpsConvertList.fromList(list1)
        val number2 = OpsConvertList.fromList(list2)
        val result1 = number1.exists(_ > 30)
        val result2 = number2.exists(_.size < 5)
        val equals1 = assert(result1)(equalTo(list1.exists(_ > 30)))
        val equals2 = assert(result2)(equalTo(list2.exists(_.size < 5)))
        equals1 && equals2
      }
    }
  )
