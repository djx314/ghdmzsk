...
class Number1S[U, +T](tail: U => Number1[T], head: IO[U]) extends Number1[T] {
  override def method1(number2: Number2): IO[T] = head.flatMap(h => number2.method2(tail(h)))
}
object Number1S {
...
class Number1U[U, +T](tail: U => Number1[T], head: Resource[IO, U]) extends Number1[T] {
  override def method1(number2: Number2): IO[T] = head.use(h => number2.method2(tail(h)))
}
object Number1U {
...
trait Number2 {
  def method2[T](number1: Number1[T]): IO[T]
}
case class Number2S(tail: () => Number2) extends Number2 {
  override def method2[T](number1: Number1[T]): IO[T] = number1.method1(tail())
}
