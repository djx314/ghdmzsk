...
trait CountSetTable {
  ...
  case class CountSetRow(id: Int, countSet: String, isLimited: Boolean, mapping: Option[String] = None)
  ...
  implicit def GetResultCountSetRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean], e3: GR[Option[String]]): GR[CountSetRow] = GR{
    prs => import prs._
    CountSetRow.tupled((<<[Int], <<[String], <<[Boolean], <<?[String]))
  }
  ...
  class CountSet(_tableTag: Tag) extends profile.api.Table[CountSetRow](_tableTag, Some("numberdatabase"), "count_set") {
    def * = (id, countSet, isLimited, mapping).<>(CountSetRow.tupled, CountSetRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(countSet), Rep.Some(isLimited), mapping)).shaped.<>({r=>import r._; _1.map(_=> CountSetRow.tupled((_1.get, _2.get, _3.get, _4)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
